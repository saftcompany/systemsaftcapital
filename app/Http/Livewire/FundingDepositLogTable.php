...
class FundingDepositLogTable extends DataTableComponent
{
    ...
    {
        return Deposit::query()->with(['user', 'gateway']);
    }
        ...
        return [
            SelectFilter::make('Status')
                ...
                ->filter(function (Builder $builder, string $value) {
                    if ($value === '0') {
                        $builder->where('deposits.status', 1);
                    } elseif ($value === '1') {
                        $builder->where('deposits.method_code', '>=', 1000)->where('deposits.status', 1);
                    } elseif ($value === '2') {
                        $builder->where('deposits.method_code', '>=', 1000)->where('deposits.status', 2);
                    } elseif ($value === '3') {
                        $builder->where('deposits.method_code', '>=', 1000)->where('deposits.status', 3);
                    }
                }),
