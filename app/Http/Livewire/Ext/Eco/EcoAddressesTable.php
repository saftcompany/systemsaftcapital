...
class EcoAddressesTable extends DataTableComponent
{
    ...
    protected $net;
    protected $api;
    const SUPPORTED_CHAINS = ["BSC", "ETH", "KLAY", "CELO", "MATIC", "TRON", 'SOL'];
        ...
        return [
            SelectFilter::make('Active')
                ...
                ->filter(function (Builder $builder, string $value) {
                    if ($value === '1') {
                        $builder->where('eco_wallets.status', true);
                    } elseif ($value === '0') {
                        $builder->where('eco_wallets.status', false);
                    }
                }),
        ];
    }
    ...
    public function refresh_free_balance()
    {
        ...
        foreach ($this->getSelected() as $id) {
            ...
            $names .= $item->index . ', ';
            $free_balance = $rpc->getTokenBalance($item);
