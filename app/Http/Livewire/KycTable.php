            ...
            Column::make("Username", "user.username")
                ...
                ->format(
                    function ($value, $row, Column $column) {
                        if ($row->user) {
                            return '<a href="' . route('admin.users.detail', $row->userId) . '" class="badge bg-primary">' . ucfirst($row->user->username) . '</a>';
                        } else {
        ...
        return [
            SelectFilter::make('Status')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('kycs.status', $value); // Specify the table name here
                }),
        ];
    }
