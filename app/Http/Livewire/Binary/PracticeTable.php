...
class PracticeTable extends DataTableComponent
{
    ...
    {
        return PracticeLog::query()->with(['user'])->where('user_id', '!=', 1);
    }
        ...
        return [
            SelectFilter::make('Result', 'result')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('practice_logs.result', $value);
                }),
            SelectFilter::make('Status')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('practice_logs.status', $value);
                }),
            DateFilter::make('From')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('practice_logs.created_at', '>=', $value);
                }),
            DateFilter::make('To')
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('practice_logs.created_at', '<=', $value);
