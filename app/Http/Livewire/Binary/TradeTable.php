...
class TradeTable extends DataTableComponent
{
    ...
    {
        return TradeLog::query()->with(['user'])->where('user_id', '!=', 1);
    }
        ...
        return [
            SelectFilter::make('Result', 'result')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('trade_logs.result', $value);
                }),
            SelectFilter::make('Status')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('trade_logs.status', $value);
                }),
            DateFilter::make('From')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('trade_logs.created_at', '>=', $value);
                }),
            DateFilter::make('To')
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('trade_logs.created_at', '<=', $value);
