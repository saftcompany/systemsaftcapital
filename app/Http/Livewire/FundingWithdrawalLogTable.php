...
class FundingWithdrawalLogTable extends DataTableComponent
{
    ...
    {
        return Withdrawal::query()->with(['user', 'method'])->where('withdrawals.status', '!=', 0);
    }
    ...
    public function filters(): array
    {
        return [
            SelectFilter::make('Status')
                ...
                ->filter(function (Builder $builder, string $value) {
                    if ($value === '1') {
                        ...
                        $builder->where('withdrawals.status', 2);
                    } elseif ($value === '3') {
                        $builder->where('withdrawals.status', 3);
                    }
                }),
            DateFilter::make('From')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('withdrawals.created_at', '>=', $value);
                }),
            DateFilter::make('To')
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('withdrawals.created_at', '<=', $value);
