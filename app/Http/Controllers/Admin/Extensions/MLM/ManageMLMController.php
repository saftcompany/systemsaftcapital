...
class ManageMLMController extends Controller
{
    ...
    public function transaction_statusUpdate(Request $request)
    {
        ...
        try {
            ...
            if ($request->type == 'withdraw' && MLMTransactions::where('hash', $request->hash)->exists()) {
                return response()->json([
                    'success' => true,
                    'type' => 'error',
                    'message' => 'The hash already exists.',
                ]);
            }
