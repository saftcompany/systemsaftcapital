...
class DepositController extends Controller
{
    ...
    public function approve(Request $request)
    {
        ...
        $platform = getPlatform('wallet');
        $chargeAmount = $platform->deposit_fees_method == 'added' ? $deposit->charge : 0;

        $transactionAmount = ($deposit->amount - $chargeAmount) / getCoinRate($deposit->symbol);

        $wal_trx = WalletsTransactions::where('user_id', $deposit->user_id)->where('trx', $deposit->trx)->first();

        if ($wal_trx === null) {
            $notify[] = ['warning', 'No WalletsTransactions found for the given transaction'];
        ...
        $transaction = createTransaction(
            ...
            $transactionAmount,
            '+',
            'Deposit Via ' . $deposit->gateway_currency()->name,
            $deposit->trx
        );
        ...
        try {
            ...
            notify($user, 'MANUAL_DEPOSIT_ADMIN_APPROVED', $notify_data);
        } catch (Throwable $e) {
        ...
        return response()->json(
            [
                ...
                'type' => $notify[0][0],
                'message' => $notify[0][1],
                'redirect' => route('admin.deposit.list')
            ]
        );
    }
