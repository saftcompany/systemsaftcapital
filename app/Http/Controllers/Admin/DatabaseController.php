...
class DatabaseController extends Controller
{
    ...
    public function checkAndAddLastProfitCalculationColumn()
    {
        if (!Schema::hasColumn('investments_log', 'last_profit_calculation')) {
            Schema::table('investments_log', function (Blueprint $table) {
                $table->timestamp('last_profit_calculation')->nullable()->after('status');
            });

            $notify[] = ['success', 'Last Profit Calculation Column Added Successfully'];
            return back()->withNotify($notify);
        }

        $notify[] = ['success', 'Last Profit Calculation Column Already Exists'];
        return back()->withNotify($notify);
    }
