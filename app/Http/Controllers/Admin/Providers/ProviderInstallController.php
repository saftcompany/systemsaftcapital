...
class ProviderInstallController extends Controller
{
    ...
    public function index($product_id)
    {
        ...
        return view('admin.provider.install', compact('page_title', 'product', 'product_id'));
    }
    ...
    public function activater($product_id)
    {
        ...
        return view('admin.provider.activate', compact('page_title', 'product'));
    }
    private $api_url;
    private $api_key;
    ...
    public function activate_licenser(Request $request)
    {
        ...
        if (!empty($create_lic)) {
            if ($response['status']) {
                ...
                $ext = ThirdpartyProvider::where('product_id', $request->product_id)->first();
                $ext->activated = '1';
                $ext->save();
            ...
            if (is_array($db_for_import)) {
                if (!empty($db_for_import["db_host"]) && !empty($db_for_import["db_user"]) && !empty($db_for_import["db_name"])) {
                    ...
                    if (mysqli_connect_errno()) {
                        ...
                        $ext = ThirdpartyProvider::where('product_id', $product_id)->first();
                        $ext->installed = '1';
                        $version = str_replace("_", ".", $version);
