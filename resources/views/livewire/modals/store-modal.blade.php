<div tabindex="-1" aria-hidden="true" data-modal-placement="center-center"
    class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden md:inset-0 h-modal md:h-full"
    id="{{ $getModelIdString() }}">
    <div class="relative w-full h-full max-w-7xl md:h-4x1">
        <!-- Modal content -->
        <div class="modal-content">
            <form action="{{ $action }}" method="POST" enctype="multipart/form-data"
                ...
                <!-- Modal body -->
                <div class="modal-body overflow-y-auto" style="max-height: calc(78vh)">
                    {{ $slot }}
                </div>
                <!-- Modal footer -->
                <div class="modal-footer text-end">
                    @php
                        $disabled = false;
                    @endphp
                    <button type="submit" id="{{ $getModelIdString() }}btn" class="btn btn-outline-primary">
                    ...
                    <button data-modal-toggle="{{ $getModelIdString() }}" id="{{ $getModelIdString() }}close"
                        type="button" class="btn btn-outline-danger">{{ __('Close') }}</button>
                </div>
            </form>
        </div>
    </div>
