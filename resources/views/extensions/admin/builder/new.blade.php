...
@section('content')
    <div class="card">
        ...
        <form action="{{ route('admin.builder.store') }}" method="POST" enctype="multipart/form-data" id="accountUpdate">
            @csrf
            <div class="card-body space-y-5">
                <div class="grid grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" class="form-control" id="slug" name="slug" aria-label="Page slug"
                            placeholder="/page/" aria-describedby="slug" value="{{ old('slug') }}">
                        <small>{{ __('add') }} <span class="text-danger">/page/</span>
                            {{ __('before the slug') }}</small>
                    </div>
                ...
                <div>
                    ...
                    <textarea class="form-control" name="style" rows="8" id="style" placeholder="{{ __('Style') }}">{!! old('style') !!}</textarea>
                    <small>{{ __('only') }} <span class="text-danger">css</span> {{ __('allowed') }}</small>
                </div>
                <div>
                    <label class="form-label" for="content">{{ __('Content') }}</label>
                    <textarea name="content" id="content" placeholder="{{ __('Content') }}" class="form-control" rows="8">{!! old('content') !!}</textarea>
                    <small>{{ __('only') }} <span class="text-danger">html, php, styling</span>
                        {{ __('allowed') }}</small>
                </div>
                <div>
                    <label class="form-label" for="script">{{ __('Script') }}</label>
                    <textarea class="form-control" name="script" rows="8" id="script" placeholder="{{ __('Script') }}">{!! old('script') !!}</textarea>
                    <small>{{ __('only') }} <span class="text-danger">js</span> {{ __('allowed') }}</small>
                </div>
            </div>
            <div class="card-footer text-end">
                <button class="btn btn-outline-success" type="submit">{{ __('Submit') }}
                </button>
            </div>
        </form>
    </div>
