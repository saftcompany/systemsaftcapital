...
@push('modals')
    <x-partials.modals.default-modal title="{{ __('Upload Token Icon') }}"
        ...
        <div>
            ...
            <small><span
                    class="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">{{ __('Icon size') }}:
                    64px * 64px</span></small>
        </div>
    ...
    <x-partials.modals.default-modal title="{{ __('Withdraw Settings') }}"
        action="{{ route('admin.eco.blockchain.tokens.withdraw.settings', ['chain' => $network->chain, 'isMainnet' => 'false']) }}"
        submit="{{ __('Edit') }}" id="editWithdraw">
        <div class="mb-1">
    ...
    <x-partials.modals.default-modal title="{{ __('Add Token Address') }}"
        ...
        <div class="mb-1">
            ...
            <label class="form-label" for="smartContract">{{ __('Token Address') }}</label>
            <input type="text" class="form-control" name="smartContract" placeholder="" required>
            <small>{{ __('Write token address from the smart contract transaction hash') }}</small>
        </div>
    </x-partials.modals.default-modal>
    <x-partials.modals.default-modal title="{{ __('Create Virtual Currency') }}" done="refresh"
        action="{{ route('admin.eco.blockchain.tokens.register', $network->chain) }}" submit="{{ __('Register') }}"
        id="tokenRegister">
        @if ($network->chain == 'TRON')
            <div class="mb-1"><label class="form-control-label h6">{{ __('Token Type') }}</label>
        ...
        <div class="mb-1">
            ...
            <small>{{ __('Base pair for') }} {{ $network->chain }}
                {{ __('token. Transaction value will be calculated according to this base pair') }}.</small>
        </div>
        <div class="mb-1">
            <input type="hidden" name="id">
            ...
            <input type="number" class="form-control" name="base_rate" placeholder="1" min="0.00000001" step="0.00000001"
                required>
            <small>{{ __('Exchange rate of the base pair. Each unit of the created curency will represent value of baseRate*1 basePair') }}.</small>
        </div>
    </x-partials.modals.default-modal>
    @if ($wallet != null)
        <x-partials.modals.default-modal title="{{ __('Deploy Token') }}" done="refresh"
            ...
            @if ($network->chain == 'TRON')
                <div class="mb-1"><label class="form-control-label h6">{{ __('Token Type') }}</label>
                    ...
                    <small
                        class="text-warning">{{ __('Make sure you have at least 200 TRX in your wallet for deployment fees') }}
        ...
        <x-partials.modals.default-modal title="{{ __('Add Token') }}"
            ...
            <div class="grid gap-5 mx-1 sm:grid-cols-2">
                ...
                <div class="mb-1">
                    ...
                    <small class="text-warning">*{{ __('Holder address must be identical to the blockchain wallet address to use same Xpub for wallets generation') }}</small>
                </div>
            </div>
        </x-partials.modals.default-modal>
    @endif
    ...
    <div class="space-x-1">
        @if ($wallet != null)
            @if (in_array($network->chain, ['ETH', 'BSC', 'MATIC', 'KLAY', 'XDC', 'ONE', 'CELO', 'SOL', 'TRON']))
                <button type="button" data-modal-toggle="deploy" class="btn btn-outline-primary"
                    @if ($network->chain == 'SOL') disabled @endif>
                    @if ($network->chain == 'SOL')
                        {{ __('Deploy Token') }} {{ __('In Development') }}
                    @else
                        <i class="bi bi-plus-lg"></i> {{ __('Deploy Token') }}
                    @endif
                </button>
            @endif
