<template>
  <div class="rounded-md bg-white p-4 shadow-md">
    <div class="mb-4 text-lg font-bold">
      {{ $t("Your Current Investment Plan") }}
    </div>
    <div class="text-gray-700">
      <div v-if="currentPlan">
        ...
        <div>
          <span class="font-bold">{{ $t("Investment Amount") }}:</span>
          {{ currentPlan.pivot.amount }}
        </div>
        <div>
          <span class="font-bold">{{ $t("Start Date") }}:</span>
          {{ currentPlan.pivot.start_date }}
        </div>
        <div>
          <span class="font-bold">{{ $t("End Date") }}:</span>
          {{ currentPlan.pivot.end_date }}
        </div>
        <div class="mt-4">
          <span class="font-bold">{{ $t("Progress") }}:</span>
          <div class="relative pt-1">
            <div class="flex h-2 overflow-hidden rounded bg-gray-300 text-xs">
              <div
      ...
      <div v-else>{{ $t("You are not currently invested in any plans") }}.</div>
    </div>
  </div>
</template>
