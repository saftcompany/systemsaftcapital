  ...
  >
    <Modal size="2xl" @close="$emit('close-modal')">
      ...
      <template #body>
        <div class="mb-5">
          <div v-if="wallet">
            <label for="wallet_id">USDT {{ $t("Funding Wallet") }}:</label>
        ...
        <div>
          <label>
            {{ $t("Please enter the amount you wish to invest and click") }}
            "<span class="text-success font-medium">{{ $t("Invest Now") }}</span
            >"
          </label>
          <input
          ...
          />
          <small
            ><span class="text-warning font-medium">{{ $t("Min") }}</span>:
            {{ plan.min_amount }} /
            <span class="text-warning font-medium">{{ $t("Max") }}</span>:
      ...
      <template #footer>
        <div class="flex justify-end">
          <button
            ...
            @click="invest()"
          >
            <ButtonLoading :loading="loading" text="Invest Now" />
          </button>
          <button
            type="button"
