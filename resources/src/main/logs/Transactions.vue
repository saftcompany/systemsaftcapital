      ...
      >
        <template #head>
          ...
          >
            ...
            <VTh sort-key="post_balance" scope="col" class="py-3 px-6">
          ...
          <template v-if="logStore.logs.transaction.length > 0">
            ...
            >
              ...
              <td
                ...
                :class="
                  'budget ' +
                  (row.trx_type == '+' ? 'text-success' : 'text-danger')
                "
              >
                {{ row.trx_type == "+" ? "+" : "-" }}
                <toMoney
                  :num="row.currency ? row.amount : row.amount * cur_rate"
                  decimals="2"
                />
                {{ row.currency ? row.currency : cur_symbol }}
              </td>
              <td data-label="Fees" class="py-4 px-6">
                <toMoney
                  :num="row.currency ? row.charge : row.charge * cur_rate"
                  decimals="2"
                />
                {{ row.currency ? row.currency : cur_symbol }}
              </td>
