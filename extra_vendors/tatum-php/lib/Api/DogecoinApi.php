...
class DogecoinApi extends AbstractApi
{
    ...
    public function dogeGenerateAddress($xpub, $index)
    {
        ...
        $result = $this->exec(
            S::createRequest(
                ...
                self::PKG,
                "GET",
                S::parse($rPath, ["xpub" => $xpub, "index" => $index]),
                $rPath,
                [],
                $rHeaders,
                []
    ...
    public function dogeGetUTXO($hash, $index)
    {
        ...
        $result = $this->exec(
            S::createRequest(
                ...
                self::PKG,
                "GET",
                S::parse($rPath, ["hash" => $hash, "index" => $index]),
                $rPath,
                [],
                $rHeaders,
                []
     ...
     *
     * @param string $address The Dogecoin address to check the balance of
     * @throws \Tatum\Sdk\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     *
     * @return \Tatum\Model\DogeBasedBalance
     */
    public function dogeGetBalance($address)
    {
        $rHeaders = $this->_headerSelector->selectHeaders(["application/json"], []);

        // Path template
        $rPath = "/v3/dogecoin/address/balance/{address}";
        /** @var \Tatum\Model\DogeBasedBalance $result */
        $result = $this->exec(
            S::createRequest(
                $this->_caller->config(),
                self::PKG,
                "GET",
                S::parse($rPath, ["address" => $address]),
                $rPath,
                [],
                $rHeaders,
                []
            ),
            "\Tatum\Model\DogeBasedBalance"
        );
     ...
     *
     * @param string[] $addresses Array of Dogecoin addresses
     * @throws \Tatum\Sdk\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     *
     * @return \Tatum\Model\DogeBasedBalance[]
     */
    public function dogeGetMultiAddressBalance(array $addresses)
    {
        $encodedAddresses = implode(',', array_map('urlencode', $addresses));
        $rHeaders = $this->_headerSelector->selectHeaders(["application/json"], []);

        // Path template
        $rPath = "/v3/dogecoin/address/balance/batch";
        /** @var \Tatum\Model\DogeBasedBalance[] $result */
        $result = $this->exec(
            S::createRequest(
                $this->_caller->config(),
    ...
    /**
     * Get unspent UTXOs for Dogecoin by address
     *
     ...
     * @return \Tatum\Model\DogeUTXO[]
     */
    public function dogeGetUnspentUtxosByAddress($address)
    {
        $rHeaders = $this->_headerSelector->selectHeaders(["application/json"], []);
        $rPath = "/v3/data/utxos";
        $result = $this->exec(S::createRequest($this->_caller->config(), self::PKG, "GET", $rPath, $rPath, [
            "chain" => S::toQueryValue('doge'),
            "address" => S::toQueryValue($address),
            "totalValue" => S::toQueryValue(200000000000)
        ], $rHeaders, []), "\Tatum\Model\DogeUTXO[]");
