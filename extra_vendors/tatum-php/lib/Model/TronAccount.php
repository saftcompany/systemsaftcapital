...
class TronAccount extends AbstractModel
{
    ...
    {
        return $this->_data["balance"] / 1000000;
    }
    ...
    /**
     * Get TRC20 token balance for a specific token address
     *
     * @param string $tokenAddress
     * @param int $decimals
     * @return float|null
     */
    public function getTrc20Balance(string $tokenAddress, int $decimals): ?float
    {
        $balance = null;
        $decimalsMultiplier = $this->createDecimalString($decimals);

        $trc20 = $this->getTrc20();
        if ($trc20) {
            foreach ($trc20 as $value) {
                if (property_exists($value, $tokenAddress)) {
                    foreach ($value as $key => $v) {
                        return $v * $decimalsMultiplier / 1000000;
                    }
                }
            }
        }
    ...
    /**
     * Create a decimal multiplier based on the number of decimals
     *
     * @param int $decimals
     * @return float
     */
    private function createDecimalString(int $decimals): float
    {
        $decimalValue = pow(10, -$decimals);
        return number_format($decimalValue, $decimals, '.', '');
    }
}
